...
export default async function ConfiguratiesPage({
  ...
  // Haal permissies op (manager heeft altijd alles, solo user ook)
  ...
  // RLS regelt al welke configs zichtbaar zijn (own_configs_only via policy)
  let query = supabase
    .from('configurations')
            ...
            {configs.map((config) => {
              ...
              return (
                <div key={config.id} className="flex items-center gap-4 px-5 py-4 hover:bg-lx-panel-bg transition-colors first:rounded-t-[18px] last:rounded-b-[18px]">
                  ...
                  {/* Prijs — alleen tonen als inkoopprijzen zijn toegestaan */}
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    {config.status === 'ordered' ? (
                      <>
                        {/* Orderbevestiging download — alleen met inkoopprijzen-recht */}
                        {canSeePurchasePrices && (
                          <div className="relative group">
                            <a
                      ...
                      <>
                        ...
                        {/* Verwijderen: eigen configs altijd; collega-configs alleen manager */}
