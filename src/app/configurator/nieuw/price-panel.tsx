...
export const MirrorPreview = memo(function MirrorPreview({ shape, width, height, diameter, directPosition, indirectPosition, glasKleur, solMeubelHoogte, solOnderkant, lunaMeubelHoogte, lunaOnderkant, lunaAfstandLinks, lunaAfstandRechts, size = 220, showDimensions = false }: {
  ...
  if (shape === 'organic') {
    ...
    // Nieuw pad heeft bbox 163.7x134 vanaf (0,0): centreren in de 200-box
    const tf = `translate(${PAD + 18.15 * scale} ${PAD + 33 * scale}) scale(${scale})`
    const hasDirect   = directPosition !== 'geen'
    const hasIndirect = indirectPosition !== 'geen'
            ...
            {shape === 'elips' && (
              <div className="rounded-xl border border-amber-300 bg-amber-50 px-3.5 py-2.5">
                <p className="text-[11.5px] text-amber-800 leading-snug">
                  Let op: dit is een <strong>voorlopige prijs</strong> (€20/m² placeholder). De definitieve glasprijs voor de Elips wordt nog aangeleverd.
                </p>
              </div>
            )}
