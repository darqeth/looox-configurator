...
export default function AdminConfigTabs({
  ...
  function handleTab(key: string) {
    ...
    startTransition(() => {
      router.push(`/admin/configuraties${params.size ? '?' + params.toString() : ''}`)
    })
  }

  return (
    <div
      className={`flex gap-1 bg-white rounded-xl p-1 border border-black/6 shadow-sm w-fit transition-opacity ${isPending ? 'opacity-60' : 'opacity-100'}`}
    >
      {tabs.map((tab) => {
