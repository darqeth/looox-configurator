          ...
          {isForgot ? (
            forgotDone ? (
              <div className="text-center py-2">
                <svg className="w-10 h-10 text-lx-cta mx-auto mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                <p className="text-sm font-semibold text-lx-text-primary">Check je inbox</p>
                <p className="text-sm text-lx-text-secondary mt-1">Als dit e-mailadres bekend is, ontvang je een resetlink.</p>
                <button onClick={() => { setIsForgot(false); setForgotDone(false); setForgotEmail('') }} className="mt-4 text-sm text-lx-cta font-semibold hover:underline">Terug naar inloggen</button>
              </div>
            ) : (
              <>
                <h2 className="text-lg font-semibold text-lx-text-primary mb-1">Wachtwoord vergeten</h2>
                <p className="text-sm text-lx-text-secondary mb-6">Vul je e-mailadres in en we sturen je een resetlink.</p>
                <form onSubmit={handleForgotPassword} className="space-y-5">
                  <div>
                    <label htmlFor="forgot-email" className="block text-sm font-semibold text-lx-text-secondary mb-1.5">E-mailadres</label>
                    <input
                      id="forgot-email"
                      type="email"
                      required
