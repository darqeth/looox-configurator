export default function MilestonesLoading() {
  return (
    <div className="p-4 sm:p-6 lg:p-7 w-full max-w-5xl animate-pulse">
      {/* Header */}
      <div className="mb-6 space-y-1.5">
        <div className="h-6 w-44 bg-lx-divider rounded-lg" />
        <div className="h-4 w-64 bg-lx-divider rounded-lg" />
      </div>

      {/* Tabs */}
      <div className="flex gap-1 mb-6 bg-white rounded-xl p-1 border border-black/6 shadow-sm w-fit">
        <div className="h-7 w-24 bg-lx-text-primary/10 rounded-lg" />
        <div className="h-7 w-28 bg-lx-divider rounded-lg" />
      </div>

      {/* 2-koloms grid: form links, lijst rechts */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        {/* Form skeleton */}
        <div className="bg-white rounded-[18px] border border-black/6 shadow-sm p-5 space-y-4">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="space-y-1.5">
              <div className="h-3 w-24 bg-lx-divider rounded" />
              <div className="h-9 w-full bg-lx-divider rounded-xl" />
            </div>
          ))}
          <div className="h-9 w-32 bg-lx-divider rounded-xl" />
        </div>

        {/* Lijst skeleton */}
        <div className="bg-white rounded-[18px] border border-black/6 shadow-sm overflow-hidden">
          <div className="px-5 py-3.5 border-b border-lx-divider">
            <div className="h-4 w-28 bg-lx-divider rounded" />
          </div>
          <div className="divide-y divide-lx-divider">
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="flex items-center gap-3 px-5 py-4">
                <div className="flex-1 space-y-1.5">
                  <div className="h-4 w-36 bg-lx-divider rounded" />
                  <div className="h-3 w-24 bg-lx-divider rounded" />
                </div>
                <div className="h-5 w-16 bg-lx-divider rounded-lg" />
                <div className="w-7 h-7 rounded-lg bg-lx-divider" />
              </div>
            ))}
          </div>
        </div>
      </div>
    </div>
  )
}
